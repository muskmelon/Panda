...
<html xmlns="http://www.w3.org/1999/xhtml" dir="<?php bloginfo('text_direction'); ?>" xml:lang="<?php bloginfo('language'); ?>">
    <head>
        ...
        <?php
      // James: added extra code to fix jquery conflict
      ...
      wp_enqueue_script( 'jquery' );
		...
		?>
        <?php wp_head(); ?>
        <?php if ( is_front_page() && !get_option('ss_disable') ) : ?>
	...
	<body <?php echo (get_option('bg_color')) ? 'style="background-color: '.get_option('bg_color').';"' : '' ?>>
        <div class="wrapper">
            ...
            <?php if ( is_front_page() && !get_option('ss_disable') ) get_template_part('slideshow'); ?>
